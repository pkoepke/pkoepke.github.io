...
const runTests = async () => {
  ...
  for (const path of filesToCheck) {
    ...
    if (document.getElementById(`forceFailure`).checked) {
      await doOneTest(`${path} + 404`, startTime); // break the path to force a failure, for testing purposes.
    } else {
    ...
    if (shouldNotifyFailure) {
      notify(outputText); // Any response except response.ok is a failure.}
    }
...
const doOneTest = async (path, startTime) => {
  ...
  if (response.ok) {
    ...
    console.log(`Failed. path: ${path}, response: ${response}`);
    console.log(`handleFailure(path, startTime): ${handleFailure(path, startTime)}`);
...
document.addEventListener(`DOMContentLoaded`, () => {
  ...
  try { document.getElementById(`font`).addEventListener(`change`, changeFont); } catch (e) { }
  try { document.getElementById(`fontSize`).addEventListener(`change`, changeFontSize); } catch (e) { }
});