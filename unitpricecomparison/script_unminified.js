...
function storeFirstRowHeight() { // runs at window.onload, stores the height of the second row. Allows CSS transitions to work since 'auto' height is not allowed for transitions.
  var nodeList = document.getElementsByClassName('itemFirstRowDiv'); // create NodeList object of all nodes of the current Class.
  nodeList = convertNodeListToArray(nodeList);
  firstRowInitialHeight = nodeList[0].clientHeight + 'px';
}
...
function expandCollapseSecondRow() {
  ...
  if (allSecondRows[0].style.height == '0px') { // expand all if first is currently collapsed
    allSecondRows.forEach(function(currentNode) {
      currentNode.style.height = firstRowInitialHeight;
      ...
      setTimeout(function() {
        ...
        allChildElements.forEach(function(currentChildElement) {
          if (currentChildElement.nodeType == 1 && currentNode.style.height == firstRowInitialHeight) { // possible race condition here - if the setTimeout timer is shorter than the card animation time, the card won't be fully extended when this height check fires and this check will fail.
